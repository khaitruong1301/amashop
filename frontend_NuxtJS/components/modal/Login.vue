          ...
          <form v-if="islogin" action="#" method="post" @submit="checkForm">
            <section class="px-5 pt-5 rounded-b-2xl">
              ...
              <div class="m-4 flex lg:justify-between lg:flex-row flex-col items-center">
                <button v-if="!isUserLoggedIn && !isForgot" type="submit" class="rounded-xl p-3 font-semibold bg-yellow hover:bg-yellow_hover text-white lg:w-1/3 w-full">
                  {{ loginBtnLabel }}
                </button>
                <p v-if="!isForgot" class="font-medium text-blue hover:text-black cursor-pointer hover:transition-all lg:m-0 m-1" @click="forgotPass(true)">
                  Forgot Password
                </p>
              </div>
              <div v-if="isForgot" class="m-4 flex flex-col text-center items-center animation-forgot">
                ...
                <div class="flex justify-center text-center space-x-4 w-full">
                  <div class="w-full">
                    <p v-if="!isSendMail" class="rounded-xl p-3 font-semibold bg-yellow hover:bg-yellow_hover text-white w-full cursor-pointer" @click="changeIsEmail(true)">
                      Send Email
                    </p>
                    <p v-else class="rounded-xl p-3 font-semibold bg-yellow hover:bg-yellow_hover text-white w-full cursor-pointer" @click="submitForgotPass">
                      Forgot Password
                    </p>
                  </div>
                  <p class="cursor-pointer rounded-xl p-3 font-semibold bg-yellow hover:bg-yellow_hover text-white w-full" @click="forgotPass(false)">
                    Back
                  </p>
                </div>
              </div>
...
export default {
  ...
  data () {
    return {
      ...
      otpNotValidLabel: 'Valid otp required',
