    ...
    >
      ...
      <tbody slot="body" slot-scope="{displayData}">
        ...
        <tr v-for="row in displayData" :key="row._id" :ref="'tr'+row._id" class="cursor-pointer" @click="focusTable(row)">
          ...
          <td>
            <button class="bg-red rounded-md px-3 py-1 text-white hover:opacity-90 " @click="showDeleteConfirm(row._id,row.username)">
              Delete
            </button>
          </td>
        </tr>
...
export default {
  ...
  methods: {
    ...
    async getUser () {
      try {
        const usersData = await this.$api.user.getUser()
    ...
    async deleteUser (_id) {
      try {
        ...
        this.$toast.success('account deleted successfully ', { timeout: 1500 })
      } catch (error) {
