<template>
  <div class="flex flex-col order_admin">
    <v-table :data="newOrders" :hide-sort-icons="true" class="table_admin table-hover" :filters="filters"
      :current-page.sync="currentPage" :page-size="10" @totalPagesChanged="totalPages = $event">
      ...
      <tbody slot="body" slot-scope="{displayData}">
        <td><input v-model="filters.username.value" class="form-control w-[90%] h-8  px-2 border  border-[#ccc] my-2">
        </td>
        <td><input v-model="filters.fullName.value" class="form-control w-[90%] h-8  px-2 border border-[#ccc] my-2"></td>
        <td><input v-model="filters.deliveryAddress.value"
            class="form-control w-[90%] h-8  px-2 border border-[#ccc] my-2"></td>
        <td><input v-model="filters.phoneNumber.value" class="form-control w-[90%] h-8  px-2 border border-[#ccc] my-2">
        </td>
        ...
        <tr v-for="row in displayData" :key="row._id" :ref="'tr' + row._id" class="cursor-pointer"
          @click="focusTable(row)">
          ...
          <td class="flex justify-between">
            <div class="">
              <span v-if="row.status === false" class=" text-white rounded-sm text-[0.8rem] px-1"
                :class="row.cancel === true ? 'bg-grey_dark' : 'bg-yellow_hover'">
            ...
            <button ref="btn_showdetails"
              class=" font-medium text-[1rem] bg-blue rounded-md py-1 px-2 hover:opacity-80 mr-2 text-white"
